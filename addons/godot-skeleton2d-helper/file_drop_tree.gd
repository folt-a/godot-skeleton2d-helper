...
func add_item(data:Dictionary,texture:Texture = null) -> void:
	...
	if is_remove_show:
		var remove_tex: = get_theme_icon("Remove", "EditorIcons")
		item.add_button(0, remove_tex, 0)
	set_file_data(item, data)
	set_selected.bind(item,0).call_deferred()
...
func get_all_file_item()  -> Array[TreeItem]:
	var root:= get_root()
	var children:Array[TreeItem] = []
	if !root:return children
	for item in root.get_children():
	...
	return Array(children.filter(func(i): return i.get_metadata(0) == "file"),TYPE_OBJECT,&"TreeItem",null) 
...
func set_recursive(item:TreeItem, data:Dictionary) -> void:
	if item.get_metadata(0) == "dir":
		...
		return set_file_data(item, data)

func set_file_data(item:TreeItem,data:Dictionary) -> void:
...
func get_recursive(item:TreeItem) -> Variant:
	if item.get_metadata(0) == "dir":
		...
		return get_file_data(item)

func get_file_data(item:TreeItem) -> Dictionary:
	var data:Dictionary = {
		"type": "file",
		"name": item.get_text(0),
